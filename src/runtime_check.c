...
{
	if (!has_argument(global.argc, global.argv, PARAM_UNCHECK_TEMPLATE_DLL)) {
		RUNTIME_CHECK_3(ExistDirectoryA(global.template_dll_to_exe) == 0 && "missing folder: template\\dll_to_exe || or use: " PARAM_UNCHECK_TEMPLATE_DLL);
	}
	if (!has_argument(global.argc, global.argv, PARAM_UNCHECK_TEMPLATE_EXE)) {
		RUNTIME_CHECK_3(ExistDirectoryA(global.template_jar_to_dll) == 0 && "missing folder: template\\jar_to_dll || or use: " PARAM_UNCHECK_TEMPLATE_EXE);
	}
}
